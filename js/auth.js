...
async function doLogin(phone, password, role) {
  ...
  if (error) return { ok: false, error: '账号或密码错误' };

  // 从 users 表获取角色和班级信息
  const profile = await UserAPI.getCurrent();
  if (!profile) return { ok: false, error: '用户信息异常，请联系管理员' };

  // 校验角色与前端选择是否一致
  if (profile.role !== role) {
    await Auth.signOut();
    return { ok: false, error: `该账号角色为「${ROLES[profile.role]?.label || profile.role}」，请重新选择` };
  }
...
function initLoginPage() {
  ...
  document.querySelectorAll('.role-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (userInput) userInput.value = MOCK_USERS[selectedRole]?.phone || '';
  ...
  // 预填演示账号
  const userInput = document.getElementById('login-user');
  if (userInput) userInput.value = MOCK_USERS.teacher.phone;
}
